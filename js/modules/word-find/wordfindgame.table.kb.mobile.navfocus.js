...
(function (document, $, wordfind) {
    ...
    */
    var drawPuzzle = function (el, puzzle) {
      ...
      for (var i = 0, height = puzzle.length; i < height; i++) {
        var row = puzzle[i];
        output += `<tr role="row"><th scope="row" class="sr-only" id="row${i + 1}">Row ${i + 1}</th>`;
        // for each element in that row
        for (var j = 0, width = row.length; j < width; j++) {
          // each cell contains a button for keyboard/focus while keeping table semantics
          var letter = row[j] || '&nbsp;';
          output += '<td>';
          output += `<button class="puzzleSquare" x="${j}" y="${i}" aria-describedby="row${i+1} col${j+1}">`;
          output += letter;
          output += '</button>';
          output += '</td>';
        }
    ...
    */
    var WordFindGame = function (puzzleEl, options) {
      ...
      */
      ...
      // Focus navigation mode state (row vs column cycling on same letter)
      var focusMode = 'row'; // 'row' or 'column'
      var focusAnchor = null; // element (button) that anchors the mode
  
      // Touch/tap state
      var touchMoved = false;
      var touchStartTarget = null;
      var suppressClickUntil = 0; // timestamp to ignore synthetic clicks after touch
      ...
      */
      ...
      var touchStart = function(e) {
        touchMoved = false;
        touchStartTarget = $(e.currentTarget).closest('.puzzleSquare').get(0);
      };
  
      var touchEnd = function(e) {
        // If we handled a tap (no movement), run click-like activation and suppress the synthetic click.
        if (!touchMoved) {
          e.preventDefault();
          e.stopPropagation();
          if (touchStartTarget) {
            // Reuse the click activation path// Reuse the click activation path; mark synthetic so it bypasses suppression
          clickActivate({ currentTarget: touchStartTarget, synthetic: true, preventDefault: function(){}, stopPropagation: function(){} });
          // Now suppress the follow-up synthetic click for a short window
          suppressClickUntil = Date.now() + 500;
          }
        } else {
          // Drag path: finish normally
          mouseUp.call(e.currentTarget);
        }
  ...
  var touchMove = function(e) {
        ...
        touchMoved = true;
        // Ensure selection started when user drags
        if (!startSquare && targetElement) {
          var sq0 = $(targetElement).closest('.puzzleSquare').get(0);
          if (sq0) {
            mouseDown.call(sq0);
          }
        }
        if (targetElement) {
          var sq = $(targetElement).closest('.puzzleSquare').get(0);
          if (sq) {
            select(sq);
            return;
          }
        }
        // Fallback: attempt with the raw element
      ...
      */
      var mouseUp = function () {
        ...
        for (var i = 0, len = wordList.length; i < len; i++) {
          ...
          if (wordList[i] === curWord) {
            ...
            if (wordList.length) {
              announce('Found ' + curWord + '. ' + wordList.length + ' words remaining.');
            } else {
              announce('Found ' + curWord + '. Puzzle complete.');
            }
          }
      ...
      var clickActivate = function (e) {
        // Allow synthetic calls from touch to bypass suppression
        if ((!e || !e.synthetic) && Date.now() < suppressClickUntil) { return; }
  
        // === Focus-mode toggle on repeated activation of the same square ===
        var target = $(e.currentTarget).closest('.puzzleSquare').get(0);
  
        // If user activated same square consecutively (3-state cycle: select -> column mode -> row mode)
        if (focusAnchor === target) {
          if (focusMode === 'row') {
            // Switch to column mode
            var x = ($(target).attr('x') - 0);
            setColumnTabbability(x);
            focusMode = 'column';
            announce('You can now focus on letters in the same column by using the tab key or swiping left and right.');
            // Do not alter selection on this toggle
            return;
          } else if (focusMode === 'column') {
            // Switch back to row mode
            resetTabbability();
            focusMode = 'row';
            announce('You can now focus on letters in the same row by using the tab key or swiping left and right.');
            // Do not alter selection on this toggle
            return;
          }
        } else {
          // New anchor: reset tabbability and set row mode
          resetTabbability();
          focusMode = 'row';
          focusAnchor = target;
        }
        // === End focus-mode toggle ===
        ...
        if (!startSquare) {
          ...
          announce(`Selected letter ${letter} - row ${y}, column ${x}`);
        } else {
      ...
      var keyActivate = function (e) {
        ...
        // Toggle focus mode if user activates the same square again via keyboard
        var kTarget = $(e.currentTarget).closest('.puzzleSquare').get(0);
        if ((isEnter || isSpace) && focusAnchor === kTarget) {
          if (focusMode === 'row') {
            var kx = ($(kTarget).attr('x') - 0);
            setColumnTabbability(kx);
            focusMode = 'column';
            announce('You can now focus on letters in the same column by using the tab key or swiping left and right.');
            e.preventDefault();
            return;
          ...
          // New anchor on first activation with keyboard
          focusAnchor = kTarget;
          resetTabbability();
          focusMode = 'row';
        }
      ...
      // Set focusable only on the given column (by x index)
      var setColumnTabbability = function(x) {
        console.log('xxx');
        var $all = $('.puzzleSquare');
        $all.attr({'tabindex':'-1','aria-hidden':'true'});
        var $col = $('.puzzleSquare[x="' + x + '"]');
        $col.each(function(){
          $(this).attr({'tabindex':'0','aria-hidden':'false'});
        });
      };
  
      // Reset tabbability to default (all focusable; remove overrides)
      var resetTabbability = function() {
        var $all = $('.puzzleSquare');
        $all.removeAttr('tabindex').removeAttr('aria-hidden');
      };
  ...
  /* Constructor START */
      ...
      if (window.navigator.msPointerEnabled) {
        $('.puzzleSquare').on('MSPointerDown', mouseDown);
        $('.puzzleSquare').on('MSPointerOver', select);
        $('.puzzleSquare').on('MSPointerUp', mouseUp);
      } else {
        $('.puzzleSquare').mousedown(mouseDown);
        $('.puzzleSquare').mouseenter(mouseMove);
        $('.puzzleSquare').mouseup(mouseUp);
